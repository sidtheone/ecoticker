...
export default function BiggestMovers() {
  ...
  return (
    <div data-testid="biggest-movers">
      <h2 className="text-lg font-semibold text-gray-200 mb-3">Biggest Movers</h2>
      <div className="flex gap-3 overflow-x-auto pb-2">
        {movers.map((m) => (
          <Link
            key={m.slug}
            href={`/topic/${m.slug}`}
            className="flex-shrink-0 bg-gray-900 border border-gray-800 rounded-lg px-4 py-3 hover:border-gray-600 transition-colors min-w-[160px]"
            data-testid="mover-card"
          >
            <div className="text-sm font-medium text-gray-200 truncate">{m.name}</div>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-xl font-bold text-gray-100">{m.currentScore}</span>
