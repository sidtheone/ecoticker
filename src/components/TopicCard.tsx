...
function getScoreColorClass(score: number): string {
  if (score >= SCORE_THRESHOLDS.BREAKING) return "text-red-500";
  if (score >= SCORE_THRESHOLDS.CRITICAL) return "text-orange-500";
  if (score >= SCORE_THRESHOLDS.MODERATE) return "text-yellow-500";
  return "text-green-500";
}

export default function TopicCard({ topic }: { topic: Topic }) {
  const scoreColor = getScoreColorClass(topic.currentScore);
  const changeDirection = topic.change > 0 ? "increased" : topic.change < 0 ? "decreased" : "unchanged";
  ...
  return (
    <Link
      ...
      aria-label={`${topic.name}. Score: ${topic.currentScore}, ${changeDirection}${changeAbs ? ` by ${changeAbs}` : ""}. ${topic.urgency} urgency.`}
    >
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {topic.sparkline && topic.sparkline.length >= 2 && (
          <Sparkline data={topic.sparkline} color={SCORE_COLORS[scoreColor]} />
        )}
      </div>
    </Link>
  );
